...
### `setting/admin/gpg/index_creation_time`
- **Purpose**: Records the key's original creation epoch (stored as `gpg_creation_timestamp`) when indexing public keys
- **Values**: 0/1 (default 1)
- **Source Files**:
  - `default/template/perl/gpgpg.pl`: Creation timestamp lookup and attribute write
  - `default/template/perl/index_text_file.pl`: Triggers key parsing
...
### `setting/admin/index/text_binary_guard`
- **Purpose**: Skips `.txt` files that appear binary before attempting to index them
- **Values**: 0/1 (default 1)
- **Source Files**:
  - `default/template/perl/index_file.pl`: Text indexing guard
  - `default/template/perl/utils.pl`: Binary sampling helper

### `setting/admin/index/mime_type_detection`
- **Purpose**: Uses the `file(1)` utility to detect MIME types for `.txt` files after the binary guard
- **Values**: 0/1 (default 0)
- **Source Files**:
  - `default/template/perl/index_file.pl`: MIME-based rerouting
  - `default/template/perl/utils.pl`: MIME lookup helper

### `setting/admin/footer_separator/enable`
- **Purpose**: Enables appending runtime footer separator blocks (`\n-- \n`) for newly produced items through canonical helper APIs
- **Values**: 0/1 (default 1)
- **Backward Compatibility**: Turning this off affects only new writes; existing items that already contain separator footers continue to parse/render with legacy handling.
- **Source Files**:
  - `default/template/php/utils.php`: PHP append helper gate
  - `default/template/perl/utils.pl`: Perl append helper gate
  - `default/template/php/store_new_comment.php`: primary producer uses helper
  - `default/template/php/post.php`: batch/parameter producer paths use helper
  - `default/template/php/route.php`: system metadata producer paths use helper
  - `default/template/perl/script/access_log_read.pl`: access-log producer path uses helper
  - `default/template/perl/dialog/toolbox_item_publish.pl`: publish fallback producer path uses helper
