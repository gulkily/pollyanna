...
// admin/php/route_enable is true
	...
	if ($_GET) {
		...
		if (isset($_GET['path'])) {
			...
			if ($path == '/upload.html') {
				if (GetConfig('setting/admin/php/route_upload_require_cookie')) {
					if (isset($cookie) && $cookie) {
						#ok
					} else {
						#$path = '/session.html';
			...
			if (
				...
				if ($path) {
						...
						if (isset($_GET['message'])) {
							...
							if (preg_match('/^[a-f0-9]{8}$/', $messageId)) {
								...
								if ($_SERVER['REQUEST_METHOD'] == "HEAD") {
									$serverResponse = RetrieveServerResponse($messageId, 0);
								}
								else {
									$serverResponse = RetrieveServerResponse($messageId, 1);
								}
					...
					if (GetConfig('setting/admin/php/route_show_cookie') && !$skipPrintedNotice) {
						...
						$cookieNotice = '<a href="/session.html">You are signed in with a cookie.</session>';
					...
					if (GetConfig('setting/admin/php/route_notify_printed_time') && !$skipPrintedNotice) { # route.php -- page printed time notice
						...
						$printedNotice = '<span class=advanced>' . AddAttributeToTag(GetDialogX($printedNotice, 'PrintedInfo', '', '', ''), 'table', 'id', 'PrintedInfo') . '</span>';
		...
		if ($lightMode) { # $lightMode $lightMode $lightMode $lightMode $lightMode
