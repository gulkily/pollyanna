...
# bookmark
	...
	# my @validPages =
	# valid pages
	my @simplePages = qw(history nocookie queue menu inspector data cloud bookmark help example topics access welcome judge calendar profile upload links post cookie chat thanks examples about faq documentation network schedule people donate session);
	...
	if (0) { } # this is to make all the elsifs below have consistent formatting
		...
		if ($pageType eq 'chain') { # chain.html
			# sub GetChainPage {
			$params{'dialog_columns'} = 'special_title_labels_list,chain_order,chain_timestamp,add_timestamp,file_hash,tagset_chain,cart';
		}
