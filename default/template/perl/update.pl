...
system('./default/template/perl/script/access_log_read.pl --all');


# index any files which haven't been indexed already
#system('./index.pl --all');

system('time find html/image -cmin -100 | grep \\\\.txt$ | head -n 5 | xargs ./index.pl');
system('time find html/txt -cmin -100 | grep \\\\.txt$ | head -n 5 | xargs ./index.pl');

#system('./hike.sh frontend');
#
