...
sub SqlForWeb { # replaces some spaces with &nbsp; to preserve text-based layout for html display; $text
	...
	my @sqlKeywords = qw(SELECT FROM SELECT COUNT AS MAX AS SUM AS SUM AS FROM WHERE JOIN IN SELECT FROM GROUP BY WHERE AND LIKE ORDER BY DESC LIMIT SUBSTR DATETIME UNION ON ALL LOWER UPPER);
